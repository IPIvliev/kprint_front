...
describe('seo router policy', () => {
  it('manages canonical and robots tags and applies noindex for panel routes', () => {
    ...
    expect(source).toContain('mapped.title || (to.meta && to.meta.title) || DEFAULT_TITLE')
    expect(source).toContain('mapped.description || (to.meta && to.meta.description) || DEFAULT_DESCRIPTION')
    ...
    expect(source).not.toContain('window.__PRERENDER_INJECTED')
  })
})
