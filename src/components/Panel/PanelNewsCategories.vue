          ...
          <div class="col-xxl-9 col-xl-8">
            <div class="panel__block">
              <div class="panel__head">
                <div class="panel__title">Категории новостей<span>{{ filteredCategories.length }}</span></div>
                <div class="panel__search d-none d-lg-block">
                  <div class="input">
                    <input
                      id="searchNewsCategories"
                      v-model="searchTerm"
                      type="text"
                      placeholder="Поиск по названию, slug или описанию"
                    >
                    <label class="panel__search-icon" for="searchNewsCategories">
              ...
              <div class="panel__body">
                <div class="panel__search d-block d-lg-none">
                  <div class="input">
                    <input
                      id="searchNewsCategoriesMob"
                      v-model="searchTerm"
                      type="text"
                      placeholder="Поиск по названию, slug или описанию"
                    >
                    <label class="panel__search-icon" for="searchNewsCategoriesMob">
                      <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M7.5 12.75C10.3995 12.75 12.75 10.3995 12.75 7.5C12.75 4.60051 10.3995 2.25 7.5 2.25C4.60051 2.25 2.25 4.60051 2.25 7.5C2.25 10.3995 4.60051 12.75 7.5 12.75Z" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                        <path d="M15.75 15.75L11.25 11.25" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                      </svg>
                  ...
                  <table>
                    <tr v-for="category in filteredCategories" :key="categoryKey(category)">
                      ...
                      <td>
                        ...
                        <span class="panel__table-text">{{ categoryArticlesCount(category.id) }}</span>
                      </td>
                      <td>
                        <span class="panel__table-subtitle">Цвет:</span>
                        <span class="panel__table-color">
                          <span class="panel__table-color-swatch" :style="{ backgroundColor: normalizeColor(category.color) }"></span>
                          <span class="panel__table-text">{{ normalizeColor(category.color) }}</span>
                        </span>
                      </td>
                      <td>
      ...
      <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="panel__formrow">
              <label>Цвет</label>
              <div class="panel__color-picker">
                <input
                  v-model="form.color"
                  type="color"
                  class="form-control form-control-color"
                  @change="form.color = normalizeColor(form.color)"
                >
                <div class="panel__color-palette">
                  <button
                    v-for="hex in colorPresets"
                    :key="hex"
                    type="button"
                    class="panel__color-swatch"
                    :class="{ 'panel__color-swatch--active': normalizeColor(form.color) === hex }"
  ...
  methods: {
    async fetchData () {
      ...
      try {
        ...
        this.error = err.userMessage || 'Не удалось загрузить категории новостей'
      } finally {
        this.loading = false
      }
    },
    ...
    async saveCategory () {
      ...
      try {
        ...
        if (this.isEditing && this.currentId) {
          await updatePanelArticleCategory(this.currentId, payload)
        } else {
          await createPanelArticleCategory(payload)
        }
        this.closeModal()
    ...
    async removeCategory (category) {
      ...
      this.error = ''
      try {
        await deletePanelArticleCategory(id)
        await this.fetchData()
      } catch (err) {
