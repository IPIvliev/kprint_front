          ...
          <div class="col-xxl-9 col-xl-8">
            <div class="panel__block">
              <div class="panel__head">
                <div class="panel__title">Фильтры<span>{{ filteredAttrs.length }}</span></div>
                <div class="panel__search d-none d-lg-block">
                  <div class="input">
                    <input
                      id="searchFilterAttrs"
                      type="text"
                      v-model="searchTerm"
                      placeholder="Поиск по названию"
                    >
                    <label class="panel__search-icon" for="searchFilterAttrs">
                ...
                <div class="panel__add-btn">
                  <div class="btn btn--red btn--big" @click="openCreate">
                    ...
                    <div class="d-none d-md-block">Добавить фильтр</div>
                  </div>
                </div>
              </div>
              <div class="panel__body">
                <div class="panel__formrow" style="display: flex; gap: 12px; flex-wrap: wrap;">
                  <div style="min-width: 220px;">
                    ...
                    <select v-model="filters.category" :class="['form-control', { 'panel__filter--active': filters.category }]">
                  ...
                  <div style="min-width: 180px;">
                    <label>Тип</label>
                    <select v-model="filters.type" :class="['form-control', { 'panel__filter--active': filters.type }]">
                      <option value="">Все</option>
                      <option value="C">Чек лист</option>
                      <option value="R">Диапазон</option>
                    </select>
                  </div>
                </div>
                <div class="panel__search d-block d-lg-none">
                  <div class="input">
                    <input
                      id="searchFilterAttrsMob"
                      type="text"
                      v-model="searchTerm"
                      placeholder="Поиск по названию"
                    >
                    <label class="panel__search-icon" for="searchFilterAttrsMob">
                      <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M7.5 12.75C10.3995 12.75 12.75 10.3995 12.75 7.5C12.75 4.60051 10.3995 2.25 7.5 2.25C4.60051 2.25 2.25 4.60051 2.25 7.5C2.25 10.3995 4.60051 12.75 7.5 12.75Z" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                        <path d="M15.75 15.75L11.25 11.25" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                      </svg>
                ...
                <div class="panel__table">
                  <table>
                    <tr v-for="attr in filteredAttrs" :key="attrKey(attr)">
                      <td>
                        <span class="panel__table-title">{{ attr.title || 'Без названия' }}</span>
                      ...
                      <td>
                        ...
                        <span class="panel__table-text">{{ categoriesLabel(attr.category) }}</span>
                      </td>
                      <td>
                        <span class="panel__table-icon" @click="openEdit(attr)">
                          <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <span class="panel__table-icon" @click="removeAttr(attr)">
                          <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <div v-if="!loading && !error && !filteredAttrs.length" class="panel__table-text">
                    Ничего не найдено
                  </div>
                </div>
              </div>
      ...
      <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ isEditing ? 'Редактировать фильтр' : 'Новый фильтр' }}</h5>
          ...
          <div class="modal-body">
            ...
            <div class="panel__formrow">
              ...
              <input type="text" v-model="form.title" class="form-control" placeholder="Название фильтра">
            </div>
            <div class="panel__formrow">
              <label>Тип</label>
            ...
            <div class="panel__formrow">
              ...
              <select v-model="form.category" class="form-control" multiple>
                <option v-for="option in categories" :key="option.id" :value="option.id">
                  {{ option.title }}
                </option>
              </select>
          ...
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn--black" @click="saveAttr" :disabled="saving">
  ...
  computed: {
    filteredAttrs() {
      ...
      return this.attrs.filter((a) => {
        ...
        if (this.filters.type && String(a.type || '') !== this.filters.type) {
          return false
        }
        if (this.filters.category) {
          const cat = Array.isArray(a.category) ? a.category : []
          const has = cat.some((id) => Number(id) === Number(this.filters.category))
          if (!has) {
            return false
          }
        }
  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        this.error = err.userMessage || 'Не удалось загрузить фильтры'
      } finally {
        this.loading = false
      }
    },
    ...
    categoriesLabel(value) {
      ...
      if (Array.isArray(value)) {
        return value.map((id) => {
          const c = this.categories.find((item) => Number(item.id) === Number(id))
          return c ? c.title : id
        }).join(', ')
      }
      return String(value)
    ...
    openEdit(attr) {
      ...
      const category = Array.isArray(attr.category)
        ? attr.category.map((item) => (typeof item === 'object' ? item.id : item))
        : []
      this.form = {
        title: attr.title || '',
        type: attr.type || 'C',
    ...
    async saveAttr() {
      ...
      try {
        ...
        this.error = err.userMessage || 'Не удалось сохранить фильтр'
      } finally {
        this.saving = false
      }
    },
