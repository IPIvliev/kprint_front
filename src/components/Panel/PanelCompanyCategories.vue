          ...
          <div class="col-xxl-9 col-xl-8">
            <div class="panel__block">
              <div class="panel__head">
                <div class="panel__title">Категории компаний<span>{{ filteredCategories.length }}</span></div>
                <div class="panel__search d-none d-lg-block">
                  <div class="input">
                    <input
                      id="searchCompanyCategories"
                      v-model="searchTerm"
                      type="text"
                      placeholder="Поиск по названию, slug и описанию"
                    >
                    <label class="panel__search-icon" for="searchCompanyCategories">
                ...
                <div class="panel__table">
                  <table>
                    <tr v-for="category in filteredCategories" :key="category.id">
                      ...
                      <td>
                        <span class="panel__table-subtitle">Компаний:</span>
                        <span class="panel__table-text">{{ category.companies_count || 0 }}</span>
                      </td>
                      <td>
      ...
      <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="panel__formrow">
              <label>Slug</label>
              <input v-model="form.slug" type="text" class="form-control" placeholder="company-category">
            </div>
            <div class="panel__formrow">
...
import {
  createCompanyManagerCategory,
  deleteCompanyManagerCategory,
  fetchCompanyManagerCategories,
  updateCompanyManagerCategory
...
export default {
  ...
  methods: {
    async fetchData () {
      ...
      this.error = ''
      try {
        const response = await fetchCompanyManagerCategories()
        this.categories = Array.isArray(response.data) ? response.data : []
      } catch (err) {
        this.error = err.userMessage || 'Не удалось загрузить категории компаний'
      } finally {
        this.loading = false
      }
    },
    ...
    async saveCategory () {
      ...
      try {
        ...
        if (this.isEditing && this.currentId) {
          await updateCompanyManagerCategory(this.currentId, payload)
        } else {
          await createCompanyManagerCategory(payload)
        }
        this.closeModal()
        await this.fetchData()
      } catch (err) {
        this.error = err.userMessage || 'Не удалось сохранить категорию компаний'
      } finally {
        this.saving = false
      }
    },
    async removeCategory (category) {
      ...
      if (!confirm('Удалить категорию компаний?')) {
        return
      }
      this.error = ''
      try {
        await deleteCompanyManagerCategory(category.id)
        await this.fetchData()
      } catch (err) {
        this.error = err.userMessage || 'Не удалось удалить категорию компаний'
      }
    }
  }
}
